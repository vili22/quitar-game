  ...
  struct GameView::Private
  {
    ...
    inline Private (boost::shared_ptr<WM> wm)
      : wm(wm),
        ...
        static_renderer(new TextRenderer ("/usr/share/fonts/truetype/freefont/FreeSans.ttf", 64, proxy)),
        popup_renderer(new TextRenderer ("/usr/share/fonts/truetype/freefont/FreeSans.ttf", 128, proxy)),
        head_mesh(factory->create_head ()),
        tail_mesh(factory->create_tail ())
    {
      //original font SS_FONT  
      visible = 0;
      threshold = 0;
      curr_time = 0;
    }
